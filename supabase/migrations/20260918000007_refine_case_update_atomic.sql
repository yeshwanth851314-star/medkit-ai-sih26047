-- =============================================================================
-- Migration: 20260918000007_refine_case_update_atomic.sql
-- Description:
--   Phase C & Phase D V1 True Final Closure:
--   Support actor_id fallback in rpc_update_case_atomic when called via service role client.
-- =============================================================================
...
BEGIN
  ...
  IF v_caller_id IS NULL THEN
    v_caller_id := COALESCE(
      (p_updates->>'actor_id')::uuid,
      (p_updates->>'actorId')::uuid,
      (SELECT clinician_id FROM public.cases WHERE id = p_case_id)
    );
  END IF;

  IF v_caller_id IS NULL THEN
    RAISE EXCEPTION 'UNAUTHORIZED: Authentication or actor required for case update';
  END IF;
