...
export async function POST(
  ...
  try {
    ...
    if (!token || token.trim().length === 0) {
      return NextResponse.json({ error: "Missing invitation ID or token" }, { status: 400 });
    }

    const body = await request.json().catch(() => ({}));
    const result = await revokeRemoteInvitation(token.trim(), auth.user, body?.reason);
    ...
    console.error("POST /api/intake/invite/[token]/revoke error:", err);
