...
export default async function CaseDetailsPage({
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
      ...
      <div className="rounded-xl border border-clinical-200 bg-linear-to-br from-white to-clinical-50/40 p-6 shadow-xs space-y-3">
        ...
        <p className="text-xs text-slate-700 leading-relaxed">
          {c.assessment_plan?.summary ||
            `Clinical intake completed for ${patient?.full_name || "patient"}. Chief complaint: ${c.chief_complaint}. Synthesized from structured intake responses with clinical safety guardrails.`}
        </p>
        <div className="flex items-center gap-2 text-[11px] text-slate-500 pt-1">
          <ShieldCheck className="h-3.5 w-3.5 text-clinical-600" />
          <span>Draft generated by MedKit AI. Verify before signing. Clinician holds final diagnostic authority.</span>
        </div>
      </div>
